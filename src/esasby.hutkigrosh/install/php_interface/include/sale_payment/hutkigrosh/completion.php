    ...
    <div id="hutkigrosh_buttons" class="">
        <?php if ($configurationWrapper->isWebpayButtonEnabled()) { ?>
            ...
            <div class="alert alert-info"
                 id="hutkigrosh_message"><?= TranslatorBitrix::translate('hutkigrosh_webpay_msg_payed') ?></div>
        <?php } elseif ($webpay_status and $webpay_status == 'failed') { ?>
            <div class="alert alert-danger"
                 id="hutkigrosh_message"><?= TranslatorBitrix::translate('hutkigrosh_webpay_msg_failed') ?></div>
        <?php } ?>
            <div id="webpay">
        ...
        <?php if ($configurationWrapper->isAlfaclickButtonEnabled()) { ?>
            <div id="alfaclick">
                ...
                <a class="btn btn-success"
                   id="alfaclick_button"><?= TranslatorBitrix::translate('hutkigrosh_alfaclick_label') ?></a>
            </div>
            <script type="text/javascript" src="http://ajax.microsoft.com/ajax/jQuery/jquery-1.11.0.min.js"></script>
            <script>
                    ...
                    $('#alfaclick_button').click(function () {
                        jQuery.post('<?= $alfaclick_url ?>',
                            ...
                            if (result.trim() == 'ok') {
                                ...
                                $('#hutkigrosh_buttons').before('<div class="alert alert-info" id="hutkigrosh_message"><?= TranslatorBitrix::translate("hutkigrosh_alfaclick_msg_success") ?></div>');
                            } else {
                                $('#hutkigrosh_message').remove();
                                $('#hutkigrosh_buttons').before('<div class="alert alert-danger" id="hutkigrosh_message"><?= TranslatorBitrix::translate("hutkigrosh_alfaclick_msg_unsuccess") ?></div>');
                            }
                        })
                    })
                });
